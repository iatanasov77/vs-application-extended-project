...
import { ChoosePlan, PayPlan, PaySubscription } from '@@@@/js/includes/pricing_plan.js';
...
$( function()
{
    ...
    {
        PaySubscription( $( this ).attr( 'data-url' ), $( this ).attr( 'data-isRecurring' ) );
    });
    ...
    $( '.btnCreateSubscription' ).on( 'click', function( e )
    {
        ...
        if ( ! window.CreateSubscriptionSubmited ) {
            $.ajax({
                ...
                {
                    let redirectTo      = VsPath( response.data.redirecrUrl, {}, routes );
                    //alert( redirectTo );
